...
**달성한 기준**
- BO 화면 고유 레이아웃이 `*.module.css`에 역할명 camelCase로 분리됨. (BOScenarioList 기준 충족)
- 공통 클래스·디자인 토큰이 프로덕트와 동일하게 유지됨.
- slot / reachcaster / datashot이 현행대로 유지됨(불필요한 변경 없음).

**확정 대기 기준**
- BO 코드베이스 기획서 수령 시 프론트의 인라인 정리 작업이 발생하지 않음. (개발자 검증 후 확정)
- 코드베이스 기획서가 프로덕트 코드로 직접 편입 가능한 수준에 도달함. (나머지 BO 확산 후 확정)
