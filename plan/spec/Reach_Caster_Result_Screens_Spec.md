...
- **주요 정보** (12px, muted-foreground):
  ...
  - 브랜드/업종: 브랜드명 / 업종명
- **액션 버튼**:
  - Share (드롭다운): Copy Link, Export to Excel
...
2. **Line Chart** (통합 도달률)
   - Y축 (우측): 통합 도달률 (40~80%)
   - 색상: #1a1a1a (다크모드: #f5f5f5)
   - 스타일: Straight line, 2px 두께
...
```javascript
{
  ...
  reach: isDarkMode ? '#f5f5f5' : '#1a1a1a',
}
```
...
- [ ] 로딩 스켈레톤 UI 추가
- [ ] 토스트 메시지 (링크 복사 완료 등)
- [ ] Export 기능 실제 구현 (Excel)
- [x] AI 인사이트 추가 (SpinX for Reach Caster 구현 완료)
- [x] 시나리오 공유 기능 (Share2 드롭다운 — Copy Link, Export to Excel)
