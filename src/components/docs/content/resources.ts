...
매월 5일경 전월 데이터가 정기 업데이트됩니다. 데이터 품질 향상을 위해 과거 데이터 보정 작업이 비정기적으로 진행될 수 있습니다.

### 종합 지표와 성과 지표의 차이가 뭔가요?

종합 지표는 광고비 등을 포함한 전체 지표이며, 성과 지표는 단가 및 효율 지표만으로 구성됩니다. 종합 지표 데이터셋은 Shared Slot에서 생성할 수 없습니다.

### 데이터셋을 수정할 수 있나요?

생성된 데이터셋의 설정은 수정할 수 없습니다. 대신 "복제" 기능으로 기존 설정을 복사한 뒤 필요한 부분만 변경하여 새로 생성할 수 있습니다.

### 데이터가 나오지 않습니다.

조회조건을 세분화할수록 해당 조건에 맞는 데이터가 줄어들 수 있습니다. 핵심 조건 위주로 선택하여 조회하는 것을 권장합니다.
