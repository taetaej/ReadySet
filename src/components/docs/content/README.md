# Docs 콘텐츠 작업 가이드

## 페이지 구성 개요

### Intro 페이지 (`/docs/intro`)

Intro 페이지는 **유일하게 마크다운이 아닌 커스텀 UI 컴포넌트**로 구성됩니다.
`DocsIntroPage.tsx`에서 직접 JSX로 렌더링하며, 아래 요소를 포함합니다:

- Hero 영역 (슬로건 + 플랫폼 소개 문구)
- Solutions Grid (솔루션 카드 4개)
- Workflow Timeline (4단계 흐름도)
- SpinX Banner (AI 어시스턴트 진입점)
- Quick References (빠른 링크 목록)

> ⚠️ Intro 페이지 수정은 `DocsIntroPage.tsx` + `docs.css`에서 직접 작업합니다.
> content 폴더의 `.ts` 파일과는 무관합니다.

### 일반 콘텐츠 페이지 (`/docs/{slug}`)

Intro를 제외한 모든 페이지는 **마크다운 문자열**로 작성됩니다.
`content/*.ts` 파일에 백틱 문자열로 콘텐츠를 작성하면 마크다운 렌더러가 HTML로 변환합니다.

---

## 콘텐츠 페이지 작성 규칙

### 필수 구성 요소

각 솔루션의 **개요(Overview) 페이지**는 아래 항목을 포함해야 합니다:

| 항목 | 설명 | 예시 |
|------|------|------|
| `# 제목` | 솔루션/기능 이름 | `# Reach Caster` |
| `## {이름}란?` | 한 문단 소개 | 무엇을 하는 솔루션인지 1~2문장 |
| `## 주요 기능` | 핵심 기능 불릿 리스트 | 3~5개 항목 |
| `## 워크플로우` | 사용 흐름 (번호 리스트) | 1. 생성 → 2. 설정 → 3. 실행 → 4. 결과 확인 |
| `## 접근 권한` | 역할별 권한 테이블 | Marketer / Viewer 기준 |
| `## 관련 페이지` | 하위 페이지 링크 안내 | "자세한 내용은 아래 페이지를 참고하세요." |

### 하위 페이지 (기능 상세)

개요 외 하위 페이지는 자유 형식이되, 아래 패턴을 권장합니다:

```markdown
# 페이지 제목

간단한 도입 문장 (이 페이지에서 다루는 내용)

## 섹션 1
본문...

## 섹션 2
본문...

---

## 주의사항 / 참고
- 불릿으로 정리
```

### 문체 및 톤

- **대상 독자**: 마케터, 미디어 플래너 (비개발자)
- **문체**: 경어체 (~합니다, ~됩니다)
- **용어**: 개발 용어 지양, UI에 표시되는 라벨 기준으로 설명
- **길이**: 한 페이지당 스크롤 3~4회 이내 권장

---

## 파일 구조

```
src/components/docs/
├── docsData.ts              ← 타입 정의 + 전체 섹션 조합 (수정 거의 없음)
├── DocsLayout.tsx           ← 레이아웃 컴포넌트 (수정 거의 없음)
├── docs.css                 ← 스타일 (수정 거의 없음)
├── content/                 ← ⭐ 콘텐츠 작업 폴더 (여기서 작업)
│   ├── getStarted.ts        ← Get Started 섹션
│   ├── slotboard.ts         ← SlotBoard 섹션
│   ├── datashot.ts          ← DataShot 섹션
│   ├── disabled.ts          ← Ad Curator + Budget Optimizer (준비중)
│   ├── reachCaster.ts       ← Reach Caster 섹션
│   ├── spinx.ts             ← SpinX 섹션
│   ├── resources.ts         ← Resources 섹션
│   └── releaseNotes.ts      ← Release Notes 섹션
│
public/docs/images/          ← ⭐ 이미지 폴더
├── get-started/
│   ├── 001.png
│   └── 002.png
├── slotboard/
│   ├── 001.png
│   └── 002.png
├── datashot/
├── reach-caster/
├── spinx/
└── resources/
```

---

## 콘텐츠 작업 방법

### 1. 자기 담당 파일만 수정

각 섹션은 독립 파일이므로 **자기 담당 파일만 수정**하면 됩니다.
다른 파일을 건드릴 필요 없습니다.

### 2. 페이지 추가 방법

해당 섹션 파일의 `pages` 배열에 새 객체를 추가합니다:

```typescript
{
  id: 'reach-caster-new-page',     // 고유 ID (섹션명-페이지명)
  title: '새 페이지 제목',            // 좌측 TOC에 표시되는 이름
  slug: 'reach-caster-new-page',   // URL 경로 (/docs/reach-caster-new-page)
  updatedAt: '2026-05-28',         // 마지막 수정일 (선택)
  content: `# 제목

본문 내용...`
}
```

### 3. 마크다운 문법 (지원 범위)

| 문법 | 예시 | 비고 |
|------|------|------|
| 제목 | `# H1`, `## H2`, `### H3` | H1은 페이지 제목, H2/H3은 우측 미니 TOC에 표시 |
| 볼드 | `**텍스트**` | |
| 인라인 코드 | `` `코드` `` | |
| 리스트 | `- 항목` | 불릿 리스트 |
| 번호 리스트 | `1. 항목` | |
| 이미지 | `![설명](/docs/images/섹션명/001.png)` | public 폴더 기준 경로 |
| 링크 | `[텍스트](URL)` | |

### 4. 이미지 추가 방법

1. 이미지를 `public/docs/images/{섹션명}/` 폴더에 넣기
2. 파일명 규칙: `001.png`, `002.png`, `003.png` (시퀀스)
3. 콘텐츠에서 참조: `![설명](/docs/images/reach-caster/001.png)`

**파일명 규칙:**
```
public/docs/images/
├── get-started/001.png      ← Get Started 첫 번째 이미지
├── reach-caster/001.png     ← Reach Caster 첫 번째 이미지
├── reach-caster/002.png     ← Reach Caster 두 번째 이미지
└── spinx/001.png            ← SpinX 첫 번째 이미지
```

**이미지 문법 (확장):**

| 문법 | 동작 |
|------|------|
| `![캡션](경로)` | 기본 60% 크기, 클릭 시 확대 |
| `![캡션\|100%](경로)` | 100% 크기, 클릭 시 확대 |
| `![캡션\|40%](경로)` | 40% 크기, 클릭 시 확대 |
| `![캡션\|60%\|nozoom](경로)` | 60% 크기, 클릭 확대 비활성화 |

- 크기: `|` 뒤에 퍼센트 지정 (미지정 시 60%)
- 확대: 기본 활성화. `|nozoom` 추가 시 비활성화
- 캡션: alt 텍스트가 이미지 하단에 표시됨
- 이미지는 다크 배경 + 보더 컨테이너 안에 가운데 정렬로 표시됨

---

## 작업 시 주의사항

### ✅ DO

- 자기 담당 파일만 수정
- `slug`는 전체에서 고유해야 함 (중복 금지)
- `updatedAt`은 수정할 때마다 갱신
- 백틱 문자열(`` ` ``) 안에서 줄바꿈은 그대로 반영됨
- 커밋 전 `npx vite build`로 빌드 확인

### ❌ DON'T

- `docsData.ts` 직접 수정 (섹션 순서 변경 시에만)
- 다른 사람 담당 content 파일 수정
- `slug`에 한글 사용 (영문 kebab-case만)
- 이미지 파일명에 한글/공백 사용
- Admin 역할을 권한 표나 본문에 표기 (Admin은 시스템 관리자이므로 사용자 가이드에 노출하지 않음)
- 아이콘명, 컴포넌트명 등 개발 용어를 본문에 사용 (사용자 가이드이므로 기능 설명만)

---

## 섹션 담당 배정

| 섹션 | 파일 | 담당 |
|------|------|------|
| Get Started | `getStarted.ts` | — |
| SlotBoard | `slotboard.ts` | — |
| DataShot | `datashot.ts` | — |
| Reach Caster | `reachCaster.ts` | — |
| SpinX | `spinx.ts` | — |
| Resources | `resources.ts` | — |
| Release Notes | `releaseNotes.ts` | — |

---

## 새 섹션 추가 방법 (관리자용)

1. `content/` 폴더에 새 파일 생성 (예: `newSection.ts`)
2. `DocSection` 타입에 맞게 export
3. `docsData.ts`에서 import + `docsStructure` 배열에 추가
4. 빌드 확인

---

## 로컬 확인 방법

```bash
npm run dev
# 브라우저에서 http://localhost:3000/docs 접속
```
